...
Now we use the same data for multiple linear regression. First we create a scatterplot matrix which includes all the variables in the dataset.
...
Now we perform a multiple linear regression with `mpg` as the response and all other variables except `name` as the predictors.
...
-   The form of the linear model is: $$ y = \beta_0+ \beta_1 x_1 + \beta_2 x_2 +\epsilon$$ where the coefficients are $\beta_0=2, \beta_1=2, \beta_2=0.3$.
